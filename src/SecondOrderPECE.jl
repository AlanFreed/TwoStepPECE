...
This file solves a pair of first- and second-order ODEs:
    x′ = fₓ(model, t, x, y)             controls:  a first-order  ODE
    y″ = fᵧ(model, t, x, y, y′)         responses: a second-order ODE
subject to initial conditions of x₀, y₀, y′₀ associated with time t₀ so that
    x′₀ = fₓ(model, t₀, x₀, y₀)         controls:  a first-order  ODE
    y″₀ = fᵧ(model, t₀, x₀, y₀, y′₀)    responses: a second-order ODE
...
independent variable, typically denoting time.  Here x is a vector-valued
collection of control variables, while y and y′ are vector-valued collections
of their responses and rates, with y′ denoting dy/dt and y″ denoting d²y/dt². 
The control function fₓ is often simpler than the response function fᵧ, 
but this need not be the case.  Time t₀ is usually taken to be 0.
...
The local truncation error comes from taking a difference between corrected 
and predicted values, i.e., 
    errorₓ = ∥x_corr - x_pred∥
    errorᵧ = ∥y_corr - y_pred∥
such that for the first step of integration the local truncation error is
    errorₓ =  (h/2)∥x′_pred - x′₀∥
    errorᵧ = (h²/6)∥y″_pred - y″₀∥
while for the remaining steps of integration this error is determined to be
    errorₓ = (2h/3)∥x′_pred - 2x′_curr + x′_prev∥
    errorᵧ = (h/24)∥y′_pred + 2y′_curr - 3y′_prev
...
the first integration step, begin with the initial condition and assign
    h₀ = ∥y₀∥ / ∥y′₀∥
To help avoid a potential wind-down or wind-up instability from occurring,
constrain this interval so that dt/100 < h₀ < dt/10, and then integrate
    t₁ = t₀ + h₀
    x₁ = x₀ + h₀x′₀
    y₁ = y₀ + h₀y′₀ + (h₀²/2)y″₀
   x′₁ = fₓ(model, t₁, x₁, y₁)
   ...
   y′₁ ← y′₀ + (h₀/2)(y″₁ + y″₀)
   y″₁ ← fᵧ(model, t₁, x₁, y₁, y′₁)
Afterwords, refine this estimate for h according to the formula
    h₁ = 2|(∥y₁∥-∥y₀∥) / (∥y′₁∥+∥y′₀∥)|
constrained by dt/1000 < h₁ to help avoid a potential wind-down instability.
...
repeat
    if n == 0 and s == S then
        Advance the independent variable
            t₁ = t₀ + h
        after which predictors integrate as
            x₁  = x₀ + hx′₀
            y₁  = y₀ + hy′₀ + (h²/2)y″₀
            y′₁ = y′₀ + hy″₀
        ...
        saving
            ...
            y′_pred = y′₁
            y″_pred = y″₁
        for use when computing error.  Correctors then integrate as
            x₁  ← x₀ + (h/2)(x′₁ + x′₀)  
            y₁  ← y₀ + (h/2)(y′₁ + y′₀) - (h²/12)(y″₁ - y″₀)
            y′₁ ← y′₀ + (h/2)(y″₁ + y″₀)
        after which refined approximations for their rates are re-evaluated as
            x′₁ ← fₓ(model, t₁, x₁, y₁)
            y″₁ ← fᵧ(model, t₁, x₁, y₁, y′₁)
        whose local truncation errors advance as
            ε_curr ← 1  
            errorₓ =  (h/2)∥x′_pred - x′₀∥
            errorᵧ = (h²/6)∥y″_pred - y″₀∥
        ...
        Advance the independent variable
            t_next = t_curr + h
        The main PECE solver begins with the predictors
            x_pred  = (1/3)(4x_curr - x_prev) + (2h/3)(2x′_curr - x′_prev)
            y_pred  = (1/3)(4y_curr - y_prev) + (h/6)(3y′_curr + y′_prev) 
                    + (h²/36)(31y″_curr - y″_prev)
            y′_pred = (1/3)(4y′_curr - y′_prev) + (2h/3)(2y″_curr - y″_prev)
        with a first approximation for their rates then being evaluated as
            x′_pred = fₓ(model, t_next, x_pred, y_pred)
            y″_pred = fᵧ(model, t_next, x_pred, y_pred, y′_pred)
        saving x′_pred, y′_pred and y″_pred for computing the error.
        This PECE finishes with the correctors
        ...
        after which refined approximations for their rate are re-evaluated via
            x′_next ← fₓ(model, t_next, x_next, y_next)
            y″_next ← fᵧ(model, t_next, x_next, y_next, y′_next)
        whose local truncation error advances as
            ...
            εᵧnext = errorᵧ / max(1, ∥y_next∥)
            ε_next ← max(εₓnext, εᵧnext)
        This integrator is second-order accurate. Most importantly, Gear's
        BDF2 method, which integrates x and y′, is A stable.
    end
    ...
    if C > 2 and s > 4 with s mod 2 == 1 then
        ...
        y″_prev ← fᵧ(model, t_prev, x_prev, y_prev, y′_prev)
        t_curr  ← t_next     
        x_curr  ← x_next     
        y_curr  ← y_next  
        x′_curr ← x′_next
...
struct SecondOrderPECE <: PECE
    ...
    function SecondOrderPECE(my_fₓ::Function,       # control ODE
        ...
        if t_N > t₀
            dt = convert(Float64, (t_N-t₀)/N)
        else
            error("Final value t_N must be greater than initial value t₀.")
        end
        if length(y′₀) ≠ length(y₀)
            msg = "Lengths of vectors y and y′ in y″ = f(c,t,x,y,y′) differ."
        ...
        if 4 ≠ (only(methods(my_fₓ)).nargs - 1)
            error("Function my_fₓ must have four arguments: model, t, x, y.")
        end
        ...
        if x′₀ isa Vector{<:Real}
            ...
            error("The assigned ODE fₓ does not return a vector of reals.")
        end
        
        if 5 ≠ (only(methods(my_fᵧ)).nargs - 1)
            error("Function my_fᵧ must have five arguments: model, t, x, y, y′.")
        end
        y″₀ = my_fᵧ(my_model, t₀, x₀, y₀, y′₀)
        if y″₀ isa Vector{<:Real}
            if length(y″₀) ≠ length(y₀)
                msg = "Length of y″ differs from lengths of y and y′ in y″=fᵧ(model,t,x,y,y′)."
                throw(DimensionMismatch(msg))
            end
        else
        ...
        # To assist in avoiding wind-down or wind-up instabilities.
        if h₀ < dt/100
            h₀ = dt / 100
        end
        if h₀ > dt/10
        ...
        print("\n.")
        new(my_fₓ, my_fᵧ, my_model, N, dt, h, n, s, t₀, x₀, y₀, y′₀, 
    ...
    function SecondOrderPECE(fₓ::Function,
                             fᵧ::Function,
                             model::Model,
                             N::Integer,
                             dt::Real,
        ...
        new(fₓ, fᵧ, model, N, dt, h, n, s, t₀, x₀, y₀, y′₀, 
            t_prev, t_curr, t_next, x_prev, x_curr, x_next, 
            y_prev, y_curr, y_next, x′_prev, x′_curr, x′_next, 
            y′_prev, y′_curr, y′_next, y″_prev, y″_curr, y″_next,
            tol, ε_curr, ε_next, steps, doubled, halved, repeats, atNode)
...
function advance!(pece::SecondOrderPECE)
    ...
    # P: advance the solution with predictions of
    x_pred =  ((1/3)*(4x_curr - x_prev) + (2h/3)*(2x′_curr - x′_prev))
    y_pred  = ((1/3)*(4y_curr - y_prev) + (h/6)*(3y′_curr + y′_prev) 
    ...
    y″_pred = pece.fᵧ(pece.model, t_next, x_pred, y_pred, y′_pred)
    # C: correct the solution with corrections of
    x_next  = (1/3)*(4x_curr - x_prev) + (2h/3)*x′_pred
    y_next  = ((1/3)*(4y_curr - y_prev)
               + (h/24)*(y′_pred + 14y′_curr + y′_prev) 
               + (h*h/72)*(10y″_pred + 51y″_curr - y″_prev))
    y′_next = (1/3)*(4y′_curr - y′_prev) + (2h/3)*y″_pred
    # E: re-evaluated the ODEs
    x′_next = pece.fₓ(pece.model, t_next, x_next, y_next)
    y″_next = pece.fᵧ(pece.model, t_next, x_next, y_next, y′_next)
    
    # determine the local truncation error
    errorₓ = (2h/3)*LA.norm(x′_pred - 2x′_curr + x′_prev)
    εₓnext = errorₓ / max(1, LA.norm(x_next))
    errorᵧ = (h/24)*LA.norm(y′_pred + 2y′_curr - 3y′_prev
           + (h/3)*(10y″_pred - 11y″_curr + y″_prev))
    ...
    elseif ε_next < pece.tol    # with C ≤ 1
        ...
        y′_prev = PF.Vector(pece.y′_prev)
        y″_prev = pece.fᵧ(pece.model, t_prev, x_prev, y_prev, y′_prev)
