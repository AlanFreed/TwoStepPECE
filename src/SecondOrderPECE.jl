...
repeat
    if n == 0 and s == S then
        ...
        with a first approximation for its second-order derivative being
        ...
        for use when computing error. A corrector then integrates as
            y′_next ← (1/3)*(4y′_curr - y′_prev) 
                    + (2h/3)*y″_next   
            y_next  ← (1/3)*(4y_curr - y_prev)
                    + (h/24)*(y′_next + 14y′_curr + y′_prev) 
                    + (h²/72)*(10y″_next + 51y″_curr - y″_prev) 
...
function advance!(pece::SecondOrderPECE)
    ...
    # and then advance the solution with a prediction of
    y_next  = ((1/3)*(4pece.y_curr - pece.y_prev)
               + (pece.h/6)*(3pece.y′_curr + pece.y′_prev) 
               + (pece.h*pece.h/36)*(31pece.y″_curr - pece.y″_prev))
    y′_next = ((1/3)*(4pece.y′_curr - pece.y′_prev) 
               + (2pece.h/3)*(2pece.y″_curr - pece.y″_prev))
    # with a first approximation for its second-order derivative being
    y″_next = my_ode(PF.Real(x_next), PF.Vector(y_next), PF.Vector(y′_next))
    # saving
    y′_pred = PF.MVector(y′_next)
    y″_pred = PF.MVector(y″_next)
    # A corrector then integrates as
    y_next  = ((1/3)*(4pece.y_curr - pece.y_prev)
               + (pece.h/24)*(y′_next + 14pece.y′_curr + pece.y′_prev) 
               + (pece.h*pece.h/72)*(10y″_next + 51pece.y″_curr - pece.y″_prev))
    y′_next = ((1/3)*(4pece.y′_curr - pece.y′_prev) 
               + (2pece.h/3)*y″_next)
    # after which a refined approximation for its rate is re-evaluated via
    y″_next = my_ode(PF.Real(x_next), PF.Vector(y_next), PF.Vector(y′_next))
    # whose local truncation error advances as
    err = (pece.h/24)*PF.norm(y′_pred + 2pece.y′_curr - 3pece.y′_prev
                    + (pece.h/3)*(10y″_pred - 11pece.y″_curr + pece.y″_prev))
    ...
    elseif ε_next < pece.tol    # with C ≤ 1
        ...
        pece.y′_prev = ((1/2)*(y′_next + pece.y′_curr)
                        - (pece.h/8)*(y″_next - pece.y″_curr))
